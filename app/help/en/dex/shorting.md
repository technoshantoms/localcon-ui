...
In order to increase your exposure to CBANK and offer liquidity to BitAssets, such
...
The LocalCoin network is capable of issuing any amount of any BitAsset and lend
it out to participants given enough collateral.

 * *settlement price*: The price for 1 CBANK as it is traded on external exchanges.
...
The LocalCoin network is capable of margin calling those positions that do not
...
market rates up to the SQP and close the position. The remaining CBANK of the
...
increase, an additional amount of CBANK is locked as collateral, while reducing
...
particular bitAsset to hand it over to the LocalCoin network. After that, the
